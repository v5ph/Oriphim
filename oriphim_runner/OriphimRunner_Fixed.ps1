...
"@
            ...
            try {
                ...
                if ($result -match "IBKR_CONNECTED") {
                    ...
                    Write-Log "??  IBKR API test: $result" "WARN"
                    return $true  # TCP worked, assume API will work
                }
            } catch {
                Write-Log "??  Python IBKR test unavailable, using TCP test result" "WARN"
                return $true
            }
        } else {
            Write-Log "??  IBKR bots path not found, using TCP test result" "WARN"
            return $true
        }
        ...
        try {
            ...
            if ($resultLine) {
                ...
                try {
                    ...
                    Write-Log "??  Could not parse bot result: $jsonResult" "WARN"
                }
            }
...
function Connect-OriphimCloud {
    try {
        ...
        if (-not $apiKey -or $apiKey -eq "") {
            Write-Log "??  No API key configured - running in offline mode" "WARN"
            $Script:IsConnected = $false
            Update-SystemTray
            return $false
        }
...
function Show-SetupDialog {
    ...
    if ($apiKey -and $apiKey.Trim() -ne "") {
        ...
        Write-Log "??  Setup skipped - running in offline mode" "WARN"
    }
}
#endregion
